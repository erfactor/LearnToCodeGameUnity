...
namespace Packages._2DTileMapLevelEditor.Scripts
{
    ...
    public class LevelEditor : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            var posX = Mathf.FloorToInt(pos.x + .5f);
            var posY = Mathf.FloorToInt(pos.y + .5f);
        ...
        private void SetupUi()
        {
            ...
            if (canvas != null)
            {
                var levelEditorUi = Instantiate(LevelEditorUiPrefab, canvas.transform);
        ...
        {
            var selectedLayer = _layerFunctionality.GetSelectedLayer();
            if (!ValidPosition(posX, posY, selectedLayer)) return;
        ...
        public void ButtonClick(int tileIndex)
        {
            ...
            if (_previewTile != null) DestroyImmediate(_previewTile.gameObject);

            _previewTile = Instantiate(GetTiles()[_selectedTileIndex],
                new Vector3(Input.mousePosition.x, Input.mousePosition.y, 100),
                Quaternion.identity);
            foreach (var c in _previewTile.GetComponents<Collider2D>()) c.enabled = false;
        }
        ...
        public void CreateBlock(int value, int xPos, int yPos, int zPos, string pieceValue = null)
        {
            // Return on invalid positions
            if (!ValidPosition(xPos, yPos, zPos)) return;
            ...
            if (value != Empty)
            {
                BuildBlock(GetTiles()[value], xPos, yPos, zPos, GetLayerParent(zPos).transform, pieceValue);
            }
        }
        ...
        {
            foreach (var layerParent in _layerParents.Values) layerParent.SetActive(show);
        }
        ...
        {
            var emptyLevel = new int[Width, Height, Layers];
        ...
        private void BuildBlock(Transform toCreate, int xPos, int yPos, int zPos, Transform parent, string pieceValue = null)
        {
            //Create the object we want to create
            var newObject = Instantiate(toCreate, new Vector3(xPos, yPos, toCreate.position.z), Quaternion.identity);
            //Give the new object the same name as our tile prefab
            newObject.name = toCreate.name;
            if (newObject.name == "piece") newObject.GetComponent<PieceText>().Text = pieceValue ?? PieceNumberText;
        ...
        private void Fill(int posX, int posY, int selectedLayer, bool undoPush)
        {
            ...
            {
                if (undoPush) // Push level on undoStack since it is going to change
        ...
        private void ClickedPosition(int posX, int posY, int selectedLayer)
        {
            ...
            if (_level[posX, posY, selectedLayer] != _selectedTileIndex)
            {
                ...
                if (_level[posX, posY, selectedLayer] != Empty) DestroyBlock(posX, posY, selectedLayer);
